...
export function sites(): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      // Development-only identity; never emitted into the deployed Worker.
      const configuredEmail = loadEnv(server.config.mode, root, "SITES_LOCAL_").SITES_LOCAL_EMAIL;
      const localEmail = configuredEmail || "seedy@sites.test";
      const localFullName = configuredEmail ? "Local preview admin" : "Seedy";
